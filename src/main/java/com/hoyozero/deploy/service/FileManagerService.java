...
public class FileManagerService {
    ...
    public List<Map<String, Object>> listFiles(Long serverId, String path) {
        ...
        try {
            ...
            log.error("列出目录失败: serverId={}, path={}", serverId, path, e);
            throw new RuntimeException(formatSshError("列出目录失败", e));
        } finally {
    ...
    private static String formatSshError(String operation, Exception error) {
        Throwable root = error;
        while (root.getCause() != null && root.getCause() != root) {
            root = root.getCause();
        }
        String detail = root.getMessage();
        if (root instanceof com.jcraft.jsch.JSchException && detail != null
                && detail.toLowerCase(java.util.Locale.ROOT).contains("auth fail")) {
            return operation + ": SSH认证失败，请检查服务器用户名、密码或私钥，并确认目标服务器允许该用户登录";
        }
        if (root instanceof java.net.ConnectException || root instanceof java.net.SocketTimeoutException) {
            return operation + ": 无法连接目标服务器，请检查主机地址、SSH端口和网络连通性";
        }
        return operation + ": " + (detail == null || detail.isBlank() ? "SSH连接异常" : detail);
    }
