...
import com.hoyozero.deploy.utils.BuildUtils;
...
public class BuildExecutorService {
    ...
    @Value("${hoyozero.keep-workspace:false}")
    ...
    public void executeBuild(Long buildId) {
        ...
        try {
            ...
            try {
                ...
                if (project.getAutoDeploy() != null && project.getAutoDeploy() == 1) {
                    ...
                    if (serverIds == null || serverIds.isEmpty()) {
                        ...
                        Map<Long, Server> serverGroup = servers.stream()
            ...
            try {
                ...
                log.warn("发送构建状态回调失败: {}", notificationException.getMessage());
            }
        }
    }
    ...
    private File createWorkspaceDir(Long projectId, java.util.function.Consumer<String> logConsumer) {
        ...
        if (workspace != null && !workspace.isEmpty()) {
            ...
            if (osName.contains("win")) {
                // Windows: D:\hoyozero-deploy
                baseDir = "D:\\hoyozero-deploy";
            } else {
                // Linux/Mac: /home/hoyozero-deploy
                baseDir = "/home/hoyozero-deploy";
            }
        }
     ...
     * 在服务器上执行回退（根据备份文件名）
     */
    private void rollbackByBackupOnServer(Server server, Project project, String backupFileName, java.util.function.Consumer<String> logConsumer) throws Exception {
    ...
    public void executeRollback(Long buildId, Long targetBuildId) {
        Build build = buildService.getById(buildId);
        Build targetBuild = buildService.getById(targetBuildId);
        Project project = projectService.getById(build.getProjectId());
        ...
        try {
            ...
            logConsumer.accept("目标构建时间: " + targetBuild.getCreateTime());
            logConsumer.accept("========================================");
     ...
     * 在服务器上执行回退
     */
    private void rollbackOnServer(Server server, Project project, Build targetBuild, java.util.function.Consumer<String> logConsumer) throws Exception {
        logConsumer.accept("[回退] 连接服务器: " + server.getHost());
        ...
        try {
            ...
            // 根据目标构建的创建时间查找备份文件
            LocalDateTime createTime = targetBuild.getCreateTime();
